/*
 * Tastic RFID Thief by Fran Brown / Bishop Fox - www.bishopfox.com
 ...
 * 
 * We used an Arduino microcontroller to weaponize a commercial RFID badge reader (the HID MaxiProx 5375  bought on eBay)  effectively turning it into a custom, 
 * long-range RFID hacking tool.  This involved the creation of a small, portable PCB  (designed in Fritzing) that can be inserted into almost any commercial RFID reader to steal badge info.
 *  
 * Note, this PCB can alternatively be inserted into an Indala reader for testing  Indala Prox deployments (e.g. Indala Long-Range Reader 620).  Alternatively, the 
 * PCB could even be used to weaponize a high frequency (13.56MHz) RFID reader, such  as the iClass R90 Long Range reader.  The PCB can be inserted into any RFID reader 
 * that supports the standard Wiegand DATA0/DATA1 output (which is pretty much all of them).
 * 
 * For more info, see: http://www.bishopfox.com/resources/tools/rfid-hacking/attack-tools/
*/
...
//////////////////////////////////////
// Function to append the card value (bitHolder1 and bitHolder2) to the necessary array then tranlate that to the two chunks for the card value that will be output
